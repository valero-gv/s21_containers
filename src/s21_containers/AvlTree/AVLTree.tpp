...
Value &AVLTree<Key, Value>::Iterator::operator*() {
  ...
  return iter_node_->value_;  // тут было value - изменил из-за мапы - так не
                              // компилируется, value =/ key
}
...
typename AVLTree<Key, Value>::Node *AVLTree<Key, Value>::CopyTree(
    AVLTree::Node *node, AVLTree::Node *parent) {  // АЛГОРИТМ КОПИРОВАНИЯ
...
int AVLTree<Key, Value>::GetHeight(AVLTree::Node *node) {
  return node == nullptr
             ? -1
             : node->height_;  // У несуществуюшей ноды высота по правилам -1
                               // иначе мы возьмем высоту у существуюшей ноды
}
...
int AVLTree<Key, Value>::GetBalance(AVLTree::Node *node) {
  return node == nullptr
             ...
             : GetHeight(node->right_) -
                   GetHeight(node->left_);  // У несуществуюшей ноды баланс
                                            // фактор по правилам 0 иначе мы
                                            // считаем баланс по формуле
}

template <typename Key, typename Value>
void AVLTree<Key, Value>::SetHeight(AVLTree::Node *node) {
  node->height_ = std::max(GetHeight(node->left_), GetHeight(node->right_)) +
                  1;  // вычисление высоты для существующей ноды
}

template <typename Key, typename Value>
void AVLTree<Key, Value>::SwapValue(AVLTree::Node *a,
                                    AVLTree::Node *b) {  // Свап только значений
...
template <typename Key, typename Value>
size_t AVLTree<Key, Value>::RecursiveSize(AVLTree::Node *node) const {
...
template <typename Key, typename Value>
bool AVLTree<Key, Value>::RecursiveInsert(
    AVLTree::Node *node, const Key &key,
    Value value) {  // Рекурсивная вставка в дерево с добавленной
                    // логикой понимания произошла ли вставка на самом
                    // деле, возвращает также указатель на ноду
...
template <typename Key, typename Value>
typename AVLTree<Key, Value>::Node *AVLTree<Key, Value>::RecursiveDelete(
...
template <typename Key, typename Value>
typename AVLTree<Key, Value>::Node *AVLTree<Key, Value>::RecursiveFind(
    AVLTree::Node *node, const Key &key) const {
